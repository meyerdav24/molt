...
export async function purchase(
  ...
  // --- open the checkout ONCE and keep it live -------------------------------
  // The browser stays open across the mandate request, so the store's cart
  // endpoints are walked once per purchase instead of twice. Those endpoints
  // are what merchants throttle, and the doubled load was the biggest cause
  // of failed runs during the rehearsals.
  onProgress?.(
    'opening the store checkout with NO card and quoting the real total (30-120s; slow stores are waited out politely)',
  );
  const opened = await shopifyOpenCheckout({ ...quoteReq, evidence_dir: cfg.evidenceDir });
  if (!opened.ok) {
    return {
      status: 'failed',
      stage: opened.stage,
      ...
      message: `quote failed before any mandate or card existed: ${opened.reason} (${opened.detail})`,
    };
  }
  const session = opened.session;
  // Every path out of here must let the browser go; abandon() is a no-op
  // once commit() has closed the session itself.
  try {
    ...
    const result = await session.commit(cardPayload, cart.total_minor);
