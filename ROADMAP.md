...
- **BL-13** Publish the verify CLI to npm under a claimable name. The bare name `molt` on npm belongs to an unrelated package, so `npx molt verify` outside a checkout would download and run a stranger's code - which is why all docs say `pnpm exec molt verify` (works in the clone, fails loudly anywhere else). A published `@molt-protocol/cli` (or similar) would make offline verification a one-liner without a checkout; until then the safe copy stays.
